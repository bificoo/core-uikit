'use strict';

var _tslib = require('../../../_virtual/_tslib.js');
var jsxRuntime = require('react/jsx-runtime');
var cx = require('classnames');
var Badge_module = require('./Badge.module.scss.js');

function _interopDefaultLegacy (e) { return e && typeof e === 'object' && 'default' in e ? e : { 'default': e }; }

var cx__default = /*#__PURE__*/_interopDefaultLegacy(cx);

var Badge = function (_a) {
    var _b;
    var _c = _a.variant, variant = _c === void 0 ? "primary" : _c, _d = _a.pill, pill = _d === void 0 ? false : _d, className = _a.className, children = _a.children, props = _tslib.__rest(_a, ["variant", "pill", "className", "children"]);
    return (jsxRuntime.jsx("span", _tslib.__assign({ className: cx__default['default'](Badge_module.wrapper, Badge_module[variant], (_b = {}, _b[Badge_module.pill] = pill, _b), className) }, props, { children: children }), void 0));
};

module.exports = Badge;
