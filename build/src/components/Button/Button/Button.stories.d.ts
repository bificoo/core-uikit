import { Story, Meta } from "@storybook/react";
import { ButtonProps } from "components/Button";
declare const _default: Meta<import("@storybook/react").Args>;
export default _default;
export declare const Primary: Story<ButtonProps>;
export declare const PrimaryDisabled: Story<ButtonProps>;
export declare const Secondary: Story<ButtonProps>;
export declare const SecondaryDisabled: Story<ButtonProps>;
export declare const Important: Story<ButtonProps>;
export declare const ImportantDisabled: Story<ButtonProps>;
export declare const Transparent: Story<ButtonProps>;
export declare const TransparentDisabled: Story<ButtonProps>;
export declare const Block: Story<ButtonProps>;
export declare const Chinese: Story<ButtonProps>;
